...
Original file is located at
    https://colab.research.google.com/drive/1CAR8-dQzsgawvkWAdAq6nsiQwxRE-9YN
"""
...
danigoApp_X=danigoApp[['거리','Out_Temp','신호등 정지']]
danigoApp_Y=danigoApp['연료 사용량 (km)']


x_train, x_test, y_train, y_test = train_test_split(danigoApp_X, danigoApp_Y, train_size=0.7, test_size=0.3)
...
print("train모델의 오차범위는 ",score[1])

print('---------')
t_score = model.evaluate(x_test,y_test)

print("테스트모델의 오차범위는 ",t_score[1])
print('---------')
...
perPredict = my_predict/ ((134 + 97 + 99 + 110 + 118) /5 ) * 90

print('-------')
print("km소모량", my_predict)
print("%소모량 ",perPredict)
print('-----------')
...
#   pred_y = w1*danigoApp[['거리']].values.tolist()+w2*danigoApp[['Out_Temp']].values.tolist()+w3*danigoApp[['In_Temp']].values.tolist()+w4*danigoApp[['소요 시간(분)']].values.tolist()+w5*danigoApp[['신호등 정지']].values.tolist() +b
...
# pred_y_list=w1*danigoApp[['거리']].values.tolist()+w2*danigoApp[['Out_Temp']].values.tolist()+w3*danigoApp[['In_Temp']].values.tolist()+w4*danigoApp[['소요 시간(분)']].values.tolist()+w5*danigoApp[['신호등 정지']].values.tolist() +b
# print(pred_y_list) #실제 y값과 비교

# # def prediction(out_temp,dist,traffic_light):
# #   return w1*abs(out_temp)+w2*dist+w3*traffic_light+b
...
# # 구간 1-3 ( 0:18 ) 3-2 ( 21:41 ) 2-1 ( 50:66 ) 첫번째 반복
